...
int runAudioAI(float valueThreshold){
  ...
  uint32_t read_size; 
  int ret;
  static bool isRecording = true;  // startRecorderを2重でしないようにするためのフラグ
  ...
  while (1) {
    ...
    if (ret != AUDIOLIB_ECODE_OK && 
      ...
      Serial.println("here Error err = " + String(ret));
  ...
  if (pre_area < silent_th && target_area >= sound_th && post_area < silent_th) {
    ...
    DNNVariable input(frames/2*fft_samples/2);  // DNNRTの入力データ用バッファ
    ...
    value = output[index];
    // Serial.println(String(label[index]) + " : " + String(value));
  }
